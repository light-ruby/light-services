...
RSpec.describe RuboCop::Cop::Operandi::ArgumentTypeRequired, :config do
    ...
    it "registers an offense" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          arg :user_id
          ^^^^^^^^^^^^ Operandi/ArgumentTypeRequired: Argument `user_id` must have a `type:` option.
        end
      RUBY
    end
  end
  ...
  context "when arg has options but no type" do
    it "registers an offense for default option only" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          arg :params, default: {}
          ^^^^^^^^^^^^^^^^^^^^^^^^ Operandi/ArgumentTypeRequired: Argument `params` must have a `type:` option.
        end
      RUBY
    end
    ...
    it "registers an offense for optional option only" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          ...
          ^^^^^^^^^^^^^^^^^^^^^^^^^ Operandi/ArgumentTypeRequired: Argument `name` must have a `type:` option.
        end
      RUBY
    end
    ...
    it "registers an offense for context option only" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          ...
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Operandi/ArgumentTypeRequired: Argument `current_user` must have a `type:` option.
        end
      RUBY
    end
    ...
    it "registers an offense for multiple options without type" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          ...
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Operandi/ArgumentTypeRequired: Argument `status` must have a `type:` option.
        end
      RUBY
    end
  end
