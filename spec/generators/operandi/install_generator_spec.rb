...
RSpec.describe "Operandi::Generators::InstallGenerator" do
  let(:destination_root) { Dir.mktmpdir }
  let(:templates_path) do
    File.expand_path("../../../lib/generators/operandi/install/templates", __dir__)
  end
  ...
  describe "application_service.rb template" do
    ...
    it "contains correct base class inheritance" do
      ...
      expect(content).to include("class ApplicationService < Operandi::Base")
    end
  ...
  describe "initializer.rb template" do
    ...
    it "contains configuration block" do
      ...
      expect(content).to include("frozen_string_literal: true")
      expect(content).to include("Operandi.configure do |config|")
    end
