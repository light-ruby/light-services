...
RSpec.describe RuboCop::Cop::LightServices::OutputTypeRequired, :config do
    ...
    it "registers an offense" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          output :result
          ^^^^^^^^^^^^^^ LightServices/OutputTypeRequired: Output `result` must have a `type:` option.
        end
      RUBY
    end
  end
  ...
  context "when output has options but no type" do
    it "registers an offense for default option only" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          output :data, default: {}
          ^^^^^^^^^^^^^^^^^^^^^^^^^ LightServices/OutputTypeRequired: Output `data` must have a `type:` option.
        end
      RUBY
    end
    ...
    it "registers an offense for optional option only" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          ...
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ LightServices/OutputTypeRequired: Output `metadata` must have a `type:` option.
        end
      RUBY
    end
    ...
    it "registers an offense for multiple options without type" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          ...
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ LightServices/OutputTypeRequired: Output `count` must have a `type:` option.
        end
      RUBY
    end
  end
