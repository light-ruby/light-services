...
RSpec.describe RuboCop::Cop::LightServices::StepMethodExists, :config do
    ...
    it "registers an offense for missing method" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          step :validate
          ^^^^^^^^^^^^^^ LightServices/StepMethodExists: Step `validate` has no corresponding method. For inherited steps, disable this line or add to ExcludedSteps.
        end
      RUBY
    end
    ...
    it "registers an offense for each missing method" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          ...
          ^^^^^^^^^^^^^^ LightServices/StepMethodExists: Step `validate` has no corresponding method. For inherited steps, disable this line or add to ExcludedSteps.
          step :process
          ^^^^^^^^^^^^^ LightServices/StepMethodExists: Step `process` has no corresponding method. For inherited steps, disable this line or add to ExcludedSteps.
        end
      RUBY
    end
