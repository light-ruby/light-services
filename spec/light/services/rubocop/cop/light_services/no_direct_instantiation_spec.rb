...
RSpec.describe RuboCop::Cop::LightServices::NoDirectInstantiation, :config do
  ...
  context "with default ServicePattern (Service$)" do
    it "registers an offense for .new on Service class" do
      expect_offense(<<~RUBY)
        ...
        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ LightServices/NoDirectInstantiation: Use `.run`, `.run!`, or `.call` instead of `.new` for service classes.
      RUBY
    end
