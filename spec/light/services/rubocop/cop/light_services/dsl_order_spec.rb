...
RSpec.describe RuboCop::Cop::LightServices::DslOrder, :config do
    ...
    it "registers an offense when arg comes after step" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          step :process
          arg :name, type: String
          ^^^^^^^^^^^^^^^^^^^^^^^ LightServices/DslOrder: `arg` should come before `step`. Expected order: config → arg → step → output.
        end
      RUBY
    end
    ...
    it "registers an offense when config comes after arg" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          arg :name, type: String
          config raise_on_error: true
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^ LightServices/DslOrder: `config` should come before `arg`. Expected order: config → arg → step → output.
        end
      RUBY
    end
    ...
    it "registers an offense when output comes before step" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          ...
          output :result, type: Hash
          step :process
          ^^^^^^^^^^^^^ LightServices/DslOrder: `step` should come before `output`. Expected order: config → arg → step → output.
        end
      RUBY
    end
    ...
    it "registers an offense when config comes last" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          ...
          output :result, type: Hash
          config raise_on_error: true
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^ LightServices/DslOrder: `config` should come before `output`. Expected order: config → arg → step → output.
        end
      RUBY
    end
    ...
    it "registers multiple offenses for completely reversed order" do
      expect_offense(<<~RUBY)
        class MyService < ApplicationService
          ...
          ^^^^^^^^^^^^^ LightServices/DslOrder: `step` should come before `output`. Expected order: config → arg → step → output.
          arg :name, type: String
          ^^^^^^^^^^^^^^^^^^^^^^^ LightServices/DslOrder: `arg` should come before `output`. Expected order: config → arg → step → output.
          config raise_on_error: true
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^ LightServices/DslOrder: `config` should come before `output`. Expected order: config → arg → step → output.
        end
      RUBY
